...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if label:
        ...
        font_scale = 0.5
        (tw, th), _ = cv2.getTextSize(label, 0, fontScale=font_scale, thickness=tf)

        # Label background rectangle
        cv2.rectangle(im, (x1, y1 - th - 4), (x1 + tw, y1), color, -1)
        cv2.putText(im, label, (x1, y1 - 2), 0, font_scale, (255, 255, 255), thickness=tf, lineType=cv2.LINE_AA)

    # Optionally draw keypoints/skeleton
    if kpt_label and kpts is not None:
